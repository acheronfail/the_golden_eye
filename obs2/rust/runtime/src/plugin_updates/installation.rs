//! Downloads, verifies, stages, and applies plugin updates while OBS runs (`releases.rs`
//! only detects/announces). Picks the release asset, verifies vs `checksums.txt`, stages
//! into the directory supplied by the loader, then asks the loader to reload the core.
...
use crate::plugin_updates::{GithubAsset, PluginUpdate, platform_arch_suffix_for};

const CHECKSUMS_ASSET_NAME: &str = "checksums.txt";
const DOWNLOAD_TIMEOUT: Duration = Duration::from_secs(120);
const STAGED_MODULE_DATA_DIR: &str = "module-data";

/// Serializes publication to the single loader-visible staging directory.
...
/// The loader's canonical on-disk path for this core library, set by ge_core_load.
...
/// for `trigger_apply`. `assets` is reused from `releases.rs`.
pub async fn download_verify_and_stage(update: &PluginUpdate, assets: Vec<GithubAsset>) -> anyhow::Result<()> {
    ...
    // Only now touch the directory visible to the loader, replacing it wholesale.
