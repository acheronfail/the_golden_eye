...
/// Binds the listening socket synchronously so callers (e.g. `ge_runtime_start`)
...
pub async fn serve(listener: TcpListener, shutdown: oneshot::Receiver<()>, state: AppState) -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/api/v1/statistics", get(routes::statistics::handle_statistics))
        .route("/api/v1/statistics/sessions", get(routes::statistics::handle_sessions))
        .route("/api/v1/statistics/sessions/{session_id}", get(routes::statistics::handle_session))
