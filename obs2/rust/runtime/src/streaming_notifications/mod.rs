...
/// Owns the posted message so stream stop can edit the matching notification.
pub(crate) struct StreamNotifier {
    message: Mutex<Option<StreamMessage>>,
...
impl StreamNotifier {
    pub(crate) fn new(settings: Arc<SettingsStore>) -> Self {
        Self { message: Mutex::new(None), settings }
    }

    pub async fn start(&self, service_settings_json: String) {
        if let Err(e) = self.start_inner(service_settings_json).await {
    ...
    async fn stop_inner(&self) -> anyhow::Result<()> {
        ...
        let Some(message) = self.message.lock().await.take() else {
            tracing::info!("no recorded stream message to update, skipping Discord edit");
            return Ok(());
        };

        let notification_options = self.settings.get_notification_options();
    ...
    async fn start_inner(&self, service_settings_json: String) -> anyhow::Result<()> {
        let notification_options = self.settings.get_notification_options();
        if !notification_options.enabled {
