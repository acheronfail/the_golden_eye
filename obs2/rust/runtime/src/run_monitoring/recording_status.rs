...
/// A transition in the recorder's per-run state, retained in [`crate::app::AppSnapshot`] so
...
pub enum RecordingStatus {
    ...
    /// [`crate::app::AppEvent::RecordingSaved`] follows. (A failed run does this normally.)
    StatsSkipped,
    ...
    /// [`crate::app::AppEvent::RecordingSaved`] follows once the clip is written.
    SavePending,
}
...
impl RecordingStateStore {
    ...
    pub(crate) fn handle(&self, event: RecordingStateEvent) -> u64 {
        let (generation, expires_after) = {
            ...
            state.generation += 1;
            state.status = next;
            self.snapshot.update(|state| state.recording_state = next);
