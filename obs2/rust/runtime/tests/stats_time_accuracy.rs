...
/// Replaying `frame_tests/clips/kia.mp4` through the monitor must record the run's real
...
async fn first_stats_frame_misread_does_not_poison_the_saved_run_time() {
    ...
    let frames = decode_bgra_frames(&harness.root.join("frame_tests/clips/kia.mp4"));
...
/// Replaying a real RT4K capture-card clip (`frame_tests/clips/rt4kce-completed.mp4`,
...
async fn next_level_header_transition_does_not_rename_the_completed_run() {
    ...
    let start = harness.frame("frame_tests/screenshots-av2hdmi/jp - start - 7 - Secret Agent.png");
    harness.render_until_state(&start, "started").await;

    let stats = harness.frame("frame_tests/screenshots-rad2x/jp - stats - 7 - Secret Agent - 0153_0430_0144.png");
    harness.render_until_state(&stats, "savePending").await;

    let transition =
        harness.frame("frame_tests/screenshots-rad2x/jp - stats - 8 - Secret Agent - 0153 - transition.png");
    harness.obs.render(transition);
    tokio::time::sleep(Duration::from_millis(100)).await;

    let next_start = harness.frame("frame_tests/screenshots-rad2x/jp - start - 8 - Secret Agent.png");
