...
use crate::support::harness::{Harness, recording_settings};
...
async fn enabled_stream_notifications_post_once_then_edit_once() {
    ...
    unsafe { ge_runtime::ge_stream_notifier_start(service_settings.as_ptr()) };
    wait_for_calls(&calls, 1).await;
    tokio::time::sleep(Duration::from_millis(100)).await;

    ge_runtime::ge_stream_notifier_stop();
