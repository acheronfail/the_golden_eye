...
fn main() {
    ...
    // let the cbindgen-generated ge_runtime.h drift out of sync with the Rust FFI.
    ...
    let obs2_dir = crate_dir
        ...
        .expect("Rust runtime crate should live under obs2/rust/")
        .to_path_buf();
    let core_dir = obs2_dir.join("core");
    std::fs::create_dir_all(&core_dir).expect("failed to create obs2/core directory");
    let output_file = core_dir.join("ge_runtime.h");

    cbindgen::Builder::new()
        .with_crate(crate_dir)
        .with_language(cbindgen::Language::C)
        .with_include_guard("GE_RUNTIME_H")
        ...
        .exclude_item("obs_frontend_recording_start")
        .exclude_item("obs_frontend_recording_stop")
        .exclude_item("obs_frontend_replay_buffer_start")
        .exclude_item("obs_frontend_replay_buffer_stop")
        .exclude_item("obs_frontend_replay_buffer_save")
        .exclude_item("obs_frontend_replay_buffer_active")
