...
pub(crate) static REPLAY_BUFFER: ReplayCoordinator = ReplayCoordinator::new();
...
impl ReplayCoordinator {
    ...
    pub(crate) fn acquire_save(&self) -> ReplaySavePermit<'_> {
        ReplaySavePermit {
            coordinator: self,
    ...
    /// lets [`Self::handle`] tell our saves from the user's manual ones.
    ...
    fn wait_for_replay_saved(&self, since: u64, slow_warning: Duration, timeout: Duration) -> ReplaySaveWait {
        ...
        while guard.generation == since {
            ...
            if elapsed >= timeout {
                ...
                // isn't mistaken for it. `handle` holds the same lock, so a
    ...
    pub(crate) fn ensure_replay_buffer_running(&self) -> bool {
        ...
        if !crate::obs::replay_buffer_active() {
            for attempt in 1..=REPLAY_START_RETRIES {
                ...
                self.handle(ReplayEvent::Starting);
    ...
    pub(crate) fn stop_replay_buffer_if_active(&self) {
        if crate::obs::replay_buffer_active() {
            tracing::info!("stopping replay buffer");
            self.handle(ReplayEvent::Stopping);
