...
use crate::run_monitoring::publication::ReplaySaveStage;
use crate::run_monitoring::test_support::*;
use crate::run_monitoring::*;
...
fn padding_defaults_to_five_and_adds_the_internal_buffer_at_both_ends() {
    ...
    let policy = default.detection_policy();
    ...
    assert_eq!(zero.detection_policy().pre_run_padding_secs, MATCH_PADDING_BUFFER_SECS);
    assert_eq!(zero.detection_policy().post_run_padding_secs, MATCH_PADDING_BUFFER_SECS);

    let negative =
        RecordingOptions { pre_run_padding_secs: -2.0, post_run_padding_secs: -2.0, ..RecordingOptions::default() };
    assert_eq!(negative.detection_policy().pre_run_padding_secs, MATCH_PADDING_BUFFER_SECS);
    assert_eq!(negative.detection_policy().post_run_padding_secs, MATCH_PADDING_BUFFER_SECS);
}
...
fn start_then_level_screen_cancels_active_session_without_save() {
    ...
    assert_eq!(recording.run_detection.clip_start, None);
    assert_eq!(recording.run_detection.status, None);
    assert!(recording.run_detection.report.is_none());
    assert!(recording.run_detection.pending.is_none());
...
fn failed_report_then_stats_schedules_failed_save() {
    ...
    assert_eq!(recording.run_detection.status, Some(RunStatus::Failed));
    assert_eq!(recording.run_detection.report.as_ref().map(|m| m.screen), Some(Screen::Failed));
...
fn complete_report_then_stats_schedules_completed_save() {
    ...
    assert_eq!(recording.run_detection.status, Some(RunStatus::Complete));
    assert_eq!(recording.run_detection.report.as_ref().map(|m| m.screen), Some(Screen::Complete));
...
fn single_stats_frame_trusts_its_reading() {
    ...
    recording.run_detection.pending = None;
}
...
fn best_time_flicker_is_outvoted_independently_of_the_run_time() {
    // The dim best-time row flickers between 28 and a misread 20. Independent
    // voting must retain the majority 28 despite the final frame reading 20.
    let (mut recording, _events) = test_recording_saving_short_failed_runs();
    let start = Instant::now();
...
fn poll_pending_waits_for_the_padding_window_before_firing() {
    ...
    assert_eq!(fire_at, stats_at + recording.detection_policy.save_delay());
    recording.poll_pending(fire_at - Duration::from_millis(1));
    assert!(recording.run_detection.pending.is_some());
    recording.run_detection.pending = None;
}
...
fn late_save_completion_does_not_clear_a_second_runs_matching_phase() {
    ...
    recording
        .save_pipeline
        .recording_state
        .handle(crate::run_monitoring::RecordingStateEvent::SaveFinished(generation1));
    assert_eq!(recording.save_pipeline.recording_state.current(), Some(RecordingStatus::StatsSkipped));
    ...
    recording
        .save_pipeline
        .recording_state
        .handle(crate::run_monitoring::RecordingStateEvent::SaveFinished(generation2));
...
fn duplicate_start_frames_do_not_reset_the_session_anchor() {
    ...
    assert_eq!(recording.run_detection.clip_start, Some(start));
...
fn seven_options_launches_and_canonicalizes_the_completed_run() {
    ...
    let pending = pending_save_event(&mut events);
    let stats =
        recording.run_detection.pending.as_ref().and_then(|pending| pending.stats.as_ref()).expect("pending stats");
...
fn returning_to_difficulty_selection_cancels_a_007_launch() {
    ...
    assert_eq!(recording.run_detection.clip_start, None);
    assert_eq!(recording.run_detection.status, None);
    assert_eq!(recording.run_detection.identity_vote.winner, None);
    assert_eq!(recording.save_pipeline.recording_state.current(), Some(RecordingStatus::Cancelled));
}
...
fn failure_screen_variants_emit_distinct_statuses_and_save_statuses() {
    for (screen, recording_status, run_status, pending_status) in [
        ...
        assert_eq!(recording.run_detection.status, Some(run_status));
        assert_eq!(recording.run_detection.report.as_ref().map(|m| m.screen), Some(screen));
...
fn run_tracker_reports_start_without_application_dependencies() {
    let mut run_detection = RunDetection::new("en".to_owned());
    let now = Instant::now();

    let update = run_detection.on_frame(
        now,
        UNIX_EPOCH,
        &match_for_screen(Screen::Start),
        RecordingOptions::default().detection_policy(),
    );

    assert!(update.ensure_replay_buffer);
    assert_eq!(update.phase, Some(RecordingStatus::Started));
    assert_eq!(run_detection.clip_start, Some(now));
...
fn run_tracker_schedules_a_completed_run_as_a_domain_transition() {
    ...
    let policy = options.detection_policy();
    run_detection.on_frame(start, UNIX_EPOCH, &match_for_screen(Screen::Start), policy);
    run_detection.on_frame(start + Duration::from_secs(10), UNIX_EPOCH, &match_for_screen(Screen::Complete), policy);

    let update = run_detection.on_frame(finish, UNIX_EPOCH + Duration::from_secs(12), &match_with_time(), policy);

    assert_eq!(update.phase, Some(RecordingStatus::SavePending));
    assert!(update.pending_changed);
    let pending = run_detection.pending.as_ref().expect("scheduled save");
