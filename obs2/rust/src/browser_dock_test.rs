...
fn creates_dock_when_config_is_empty() {
    let output =
        ensure_dock_json(None, DOCK_TITLE, crate::config::DEFAULT_BROWSER_DOCK_URL, DOCK_UUID).unwrap().unwrap();
    let docks: Vec<Value> = serde_json::from_str(&output).unwrap();

    assert_eq!(docks.len(), 1);
    assert_eq!(docks[0]["title"], DOCK_TITLE);
    assert_eq!(docks[0]["url"], crate::config::DEFAULT_BROWSER_DOCK_URL);
...
fn leaves_existing_matching_dock_unchanged() {
    ...
    assert_eq!(
        ensure_dock_json(Some(existing), DOCK_TITLE, crate::config::DEFAULT_BROWSER_DOCK_URL, DOCK_UUID).unwrap(),
        None
    );
}
...
fn preserves_existing_docks_when_appending() {
    ...
    let output = ensure_dock_json(Some(existing), DOCK_TITLE, crate::config::DEFAULT_BROWSER_DOCK_URL, DOCK_UUID)
        .unwrap()
        .unwrap();
    let docks: Vec<Value> = serde_json::from_str(&output).unwrap();
...
fn rejects_malformed_existing_config() {
    let error = ensure_dock_json(Some("{not json"), DOCK_TITLE, crate::config::DEFAULT_BROWSER_DOCK_URL, DOCK_UUID)
        .unwrap_err();
