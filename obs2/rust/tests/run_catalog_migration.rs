...
async fn startup_seeds_missing_run_catalog_without_deleting_failed_clips() {
    let harness = Harness::start_with_settings_from_temp(Duration::ZERO, |temp| {
        ...
        recording_settings(&completed, &failed)
    })
    .await;
    ...
    assert_eq!(clips[3]["metadata"]["status"], "failed");

    let reviews: Value =
        harness.client.get(format!("{API}/api/v1/runs/review")).send().await.unwrap().json().await.unwrap();
