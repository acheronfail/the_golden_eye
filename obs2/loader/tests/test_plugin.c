// Drives the real, unmodified loader/plugin.c through its three OBS entry points
...
int main(int argc, char **argv) {
  ...
  if (g_failures == 0) {
    printf("all loader plugin tests passed\n");
    return 0;
  }
  fprintf(stderr, "%d loader plugin test failure(s)\n", g_failures);
  return 1;
}
