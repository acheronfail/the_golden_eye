...
// Small helpers shared by every obs2/loader test binary: a failure-counting
...
static inline bool test_make_temp_dir(char *out, size_t out_size) {
  ...
  char buf[PATH_MAX];
  if ((size_t)snprintf(buf, sizeof(buf), "%s/ge_loader_test_XXXXXX", (tmpdir && *tmpdir) ? tmpdir : "/tmp") >=
      sizeof(buf)) {
    return false;
  }
  if (!mkdtemp(buf)) {
