...
function colourize(text: string, offset = 0): string {
  return [...text]
    .map((ch, i) => `{${RAINBOW[(i + offset) % RAINBOW.length]}-fg}${ch}{/}`)
    .join("");
}
...
function addRainbowBorder(
  ...
  const render = () => {
    ...
    const vertical = Array.from({ length: height }, (_, i) =>
      colourize("█", i),
    ).join("\n");
    leftOuter.setContent(vertical);
    rightOuter.setContent(
      Array.from({ length: height }, (_, i) => colourize("█", i + 3)).join(
        "\n",
      ),
    );
  };
...
function createStatusBox(
  ...
  const figletText = figlet.textSync(options.headingText, {
    font: options.headingFont || "Standard",
  });
  const heading = options.rainbowHeading
    ? colourizeNonWhitespace(figletText)
    : `{${options.headingColor}-fg}${figletText}{/}`;
  const figletWidth = Math.max(
    ...figletText.split("\n").map((l) => l.length),
    options.hintText.length + 1,
  );
  const hintPadding = Math.max(
    0,
    Math.floor((figletWidth - options.hintText.length) / 2),
  );
...
export const createLevelInfoBox = (
  screen: Widgets.Screen,
  levelInfo: LevelInfo,
) => {
