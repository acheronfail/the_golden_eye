...
- Failed-clip retention (`failedRunLimit`) is enforced **globally across all roots/subdirectories** by `prune_failed_clips` (order by timestamp, keep newest N). Do not reintroduce per-directory pruning.
