...
- `obs2/rust/core/src/app/` - feature construction, cross-feature settings application, and browser publication. Feature workflows receive explicit dependencies instead of `AppState`; HTTP routes decode requests and map results. See `obs2/rust/core/src/README.md` for the ownership map.
...
- `obs2/rust/core/src/run_monitoring/` - automatic run capture. Start with `session.rs` for the workflow, `run_detection.rs` for run transitions, `in_game_timer.rs` for timer rules, and `lifecycle.rs` for starting/stopping OBS capture. See its `README.md` for the responsibility map.
